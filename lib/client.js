...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const MAX_MARK_GAP_PX = 30;   // cap the center-to-center distance between markers
    ...
    const css = [
      ...
      ".tr_rail:not(.tr_show) .tr_scroll{pointer-events:none}",
      ".tr_rail:not(.tr_show) .tr_tip{pointer-events:none}",
    ...
    function TurnRail({ useProjection, sessionId, sessionsService, t }) {
      ...
      if (messages.length === 0) {
        ...
        // 兼容旧投影缓存：缺少 id/key 的条目先从已加载节点补齐定位 key。
        const bySeq = new Map();
        for (const item of collectFromNodes(nodeSnapshot)) bySeq.set(item.seq, item);
        for (const item of messages) {
          if (anchorKeyOf(item) === void 0 && bySeq.has(item.seq)) {
            item.key = bySeq.get(item.seq).key;
          }
        }
      }
      ...
      const [viewportH, setViewportH] = React.useState(() => window.innerHeight);
      ...
      const initialPinSessionRef = React.useRef(null);
      ...
      // Scrolling rail is user-owned after the first paint:
      // - pin to the bottom once when a session opens;
      // - never move it programmatically afterwards. Click-to-jump and chat
      //   scrolling therefore cannot steal the rail's scroll position.
      React.useEffect(() => {
      ...
      const railHeightPx = viewportH * 0.64;
      const slicePx = Math.min(railHeightPx / n, MAX_MARK_GAP_PX);
      const slicePct = (slicePx / railHeightPx) * 100;
      const topFor = (i) => 50 - (n * slicePct) / 2 + i * slicePct;
      const heightFor = () => slicePct;
                ...
                return React.createElement(
                  "button",
                  {
                    ...
                    onClick: () => {
                      ...
                      setQuery("");
                      setTip(null);
                      const idx = messages.findIndex((x) => x.seq === m.seq);
